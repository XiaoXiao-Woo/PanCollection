...
class FusionNet(PatchMergeModule):
    ...
    def train_step(self, data, **kwargs):
        sr =  data["lms"] + self._forward_implem(data["lms"], data["pan"])
        loss = self.criterion(sr, data["gt"])
        return loss

    def val_step(self, data, **kwargs):
        return data["lms"] + self._forward_implem(data["lms"], data["pan"])
...
class build_fusionnet(PanSharpeningModel, name="FusionNet"):
    def __call__(self, args, logger=None):
        scheduler = None
        
        for key in ["train_name", "val_name", "test_name"]:
            if hasattr(args.dataset, key):
                if "wv" in getattr(args.dataset, key):
                    spectral_num = 8
                else:
                    spectral_num = 4
                break
        ...
        optimizer = get_optimizer(model, model.parameters(), **args.optimizer_cfg)
...
if __name__ == "__main__":
    ...
    from pancollection import pancollection_dir
    from udl_vis.tests.test_task import test_pansharpening

    test_pansharpening(Path(pancollection_dir) / "configs", build_fusionnet, PanSharpeningModel)
