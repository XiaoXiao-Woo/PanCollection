...
def train(training_data_loader, validate_data_loader, start_epoch=0):
    ...
    for epoch in range(start_epoch, epochs, 1):
        ...
        for iteration, batch in enumerate(training_data_loader, 1):
            gt, LRHS, RGB = batch[0].cuda(), batch[1].cuda(), batch[2].cuda()
            optimizer.zero_grad()
            sr = model(LRHS, RGB)
...
if __name__ == "__main__":
    train_set = DatasetFromHdf5('./training_data/train_cave.mat')
    training_data_loader = DataLoader(dataset=train_set, num_workers=0, batch_size=batch_size, shuffle=True,
                                      pin_memory=True, drop_last=True)
    validate_set = DatasetFromHdf5('./training_data/validation_cave.mat')
