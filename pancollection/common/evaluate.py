...
def analysis_accu(img_base, img_out, ratio, flag_cut_bounds=True, dim_cut=1, choices=4, prefix=""):
    if flag_cut_bounds:
    ...
    h = img_out.shape[1]
    w = img_out.shape[2]
    chanel = img_out.shape[3]

    # 计算SAM
    sum1 = torch.sum(img_base * img_out, 3)
    sum2 = torch.sum(img_base * img_base, 3)
    sum3 = torch.sum(img_out * img_out, 3)
    t = (sum2 * sum3) ** 0.5
    numlocal = torch.gt(t, 0)
    num = torch.sum(numlocal, [1, 2])
    t = sum1 / t
    angle = torch.acos(t)
    sumangle = torch.where(torch.isnan(angle), torch.full_like(angle, 0), angle).sum([1, 2])
    # if num == 0:
    #     averangle = sumangle
    # else:
    #     averangle = sumangle / num
    ...
    num = torch.where(num == 0, torch.ones_like(sumangle), num)
    averangle = sumangle / num
    ...
    summ = 0
    for i in range(chanel):
        a1 = torch.mean((img_base[..., i] - img_out[..., i]) ** 2, [1, 2])
        m1 = torch.mean(img_base[..., i], [1, 2])
        a2 = m1 * m1
        summ = summ + a1 / a2
    ERGAS = 100 * (1 / ratio) * ((summ / chanel) ** 0.5)
    ...
    # PSNR = -20 * temp
    PSNR = 10 * torch.log10(math.pow(1.0, 2) / torch.mean((img_out-img_base)**2, [1, 2]))
    ...
    if choices == 5:
        ...
        CC = C1 / ((C2 * C3) ** 0.5)
        CC = torch.mean(CC)
        return {f'{prefix}_SAM': SAM, f'{prefix}_ERGAS': ERGAS, f'{prefix}_PSNR': PSNR, f'{prefix}_CC': CC}  # , q2n_index

    return {f'{prefix}_SAM': SAM, f'{prefix}_ERGAS': ERGAS, f'{prefix}_PSNR': PSNR, }
