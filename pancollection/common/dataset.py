...
def load_dataset_singlemat_hp(file_path, scale):
    ...
    ms_hp = (
        torch.from_numpy(get_edge(data["ms"] / scale)).permute(2, 0, 1).unsqueeze(dim=0)
    )  # CxHxW= 8x64x64
    mms_hp = F.interpolate(
        ms_hp,
        size=(ms_hp.size(2) * 2, ms_hp.size(3) * 2),
        mode="bilinear",
        align_corners=True,
    )
...
class SingleDataset(Dataset):
    ...
    def __init__(self, file_lists, dataset_name, img_scale, dataset=None):
        ...
        if "hp" not in dataset_name:
            ...
            file_lists = [file_path.replace("_hp", "") for file_path in file_lists]
        ...
        if len(file_lists) == 1:
            self.test_lms, self.test_mms, self.test_ms, self.test_pan, self.gt = (
                self.dataset(file_lists[0], self.img_scale)
            )
            print(
                f"lms: {self.test_lms.shape}, ms: {self.test_ms.shape}, pan: {self.test_pan.shape}, gt: {self.gt.shape}"
            )
    ...
    def __getitem__(self, idx):
        ...
        if self.file_lists != 0:
            self.test_lms, self.test_mms, self.test_ms, self.test_pan, self.gt = (
                self.dataset(file_path, self.img_scale)
            )
...
class SingleDatasetV2(Dataset):
    ...
    def __getitem__(self, idx):
        file_path = self.file_lists[idx % self.file_nums]
        test_lms, test_mms, test_ms, test_pan, gt = self.dataset(
            file_path, self.img_scale
        )
...
def save_results(idx, save_model_output, filename, save_fmt, output):
    ...
    for fmt in save_fmt:
        for i in range(output.shape[0]):
            ...
            if fmt != "mat":
                try:
                    out = showimage8(output[i])
                except Exception as e:
                    logger.error(e)
                    out = output[..., [0, 2, 4]].cpu().detach().numpy()
                if out.max() <= 1:
